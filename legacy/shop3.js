...
wishlistButton.addEventListener("click", function () {
    ...
    wishlistProducts.forEach(product => { wishlistHTML += ` <div class="wishlist-item"> <img src="${product.image}" alt="${product.name}" width="100"> <div> <h4>${product.name}</h4> <p>₹${product.price}</p> </div> </div> `; });
...
// Remove product from wishlist
 function removeFromWishlist(id) { wishlist = wishlist.filter(productId => productId !== id); 
...
function toggleWishlist(id) {
    ...
    localStorage.setItem("novaeWishlist",JSON.stringify(wishlist));
   updateWishlistCount();
...
//         statValues[1] >= 1000
//             ? Math.floor(statValues[1] / 1000) + "K+"
//             : statValues[1];
...
counters.forEach(function (counter) {
 ...
 document.addEventListener("DOMContentLoaded", () => {
            const counters = document.querySelectorAll('.stat-number');
            const speed = 2000; // Total animation duration in milliseconds (2 seconds)
            ...
            const animateCounter = (counter) => {
                ...
                const updateCount = (currentTime) => {
                    ...
                    // Easing function (Ease-out quad) for smooth slowdown at the end
                    ...
                    if (progress < 1) {
                        ...
                        counter.innerText = target.toFixed(decimals) + suffix; // Ensure exact final value
                    }
                };
            ...
            // Setup Intersection Observer so animation plays when visible on screen
...
const storySwiper = new Swiper(".storySwiper", {
    ...
    navigation: {
        nextEl: ".storySwiper .swiper-button-next",
        prevEl: ".storySwiper .swiper-button-prev"
    },

    pagination: {
        el: ".storySwiper .swiper-pagination",
        clickable: true
    }
